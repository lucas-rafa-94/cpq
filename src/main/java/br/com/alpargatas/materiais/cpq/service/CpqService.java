...
public class CpqService {
    ...
    @Async
    public void createUpdateCpq(Item item, String id){
        logger.error("Item " + item.getItemNumber().getValue() + " sendo criado/update part no cpq");
        cpqPartServiceRequestCaller.call(cpqPartBusiness.transformItemOmToParts(item, id));
        ...
        cpqDtModeloServiceRequestCaller.postDtModelo(cpqDtModeloBusiness.transformItemOmToCustPrice(item));
    }
}
